...
# We load the "rules" (scaler) and the "brain" (model) we saved during training.
# In MLOps, these are called 'Production Artifacts'.
...
# --- STEP 3: COLLECTING RAW DATA ---
# These widgets gather the "Raw Features" from the user. 
# Note: These values are not yet scaled (e.g., Platelets could be 250,000).
...
if st.button("Heart Failure"):
    # Create a list containing all inputs in the exact order the model expects
    heart_input = [[age, anaemia, creatinine, diabetes, ef, hbp, platelets, serum_creatinine,
                        serum_sodium, sex, smoking, time]]
    
    # PRE-PROCESSING: Use the loaded scaler to transform raw data into scaled data.
    # This "translates" the large numbers so the model can understand them correctly.
    scaled = heart_scaler.transform(heart_input)
    
    # INFERENCE: Feed the scaled data into the model to get the prediction (0 or 1)
    result = heart_model.predict(scaled)
    
    # POST-PROCESSING: Translate the numeric 0/1 into a human-readable message
    if result[0] == 1:
